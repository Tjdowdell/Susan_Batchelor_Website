...
class Google
{
    ...
    private function handleEventData( Entities\Appointment $appointment )
    {
        ...
        foreach ( $appointment->getCustomerAppointments() as $ca ) {
            ...
            if ( $ca->get( 'extras' ) != '[]' ) {
                $extras = implode( ', ', array_map( function ( $extra ) { return $extra->get( 'title' ); }, apply_filters( 'bookly_extras_find_by_ids', array(), array_keys( json_decode( $ca->get( 'extras' ), true ) ) ) ) );
                if ( ! empty( $extras ) ) {
                ...
                foreach ( $events->getItems() as $event ) {
                    ...
                    if ( $event->getStatus() !== 'cancelled' && ( $event->getTransparency() === null || $event->getTransparency() === 'opaque' ) ) {
                        ...
                        if ( $eventEndDate > $startDate ) {
                            // Round event end time to a multiple of the default time slot length.
                            $extra = $eventEndDate->getTimestamp() % $time_slot_length;
                            if ( $extra ) {
                                $eventEndDate->modify( '+' . ( $time_slot_length - $extra ) . ' sec' );
                            }
                            // Round event start time like the default time slot length.
                            $extra = $eventStartDate->getTimestamp() % $time_slot_length;
                            if ( $extra ) {
                                $eventStartDate->modify( '-' . $extra . ' sec' );
                            }
                            ...
                            for ( $date = clone $eventStartDate; $date < $eventEndDate; $date->modify( '+1 day' ) ) {
                                $result[] = array(
                                    'from_google'        => true,
                                    'service_id'         => 0,
                                    'start_time'         => $date->getTimestamp() + $gmt_offset,
                                    'end_time'           => ( $eventEndDate->format( 'ymd' ) == $date->format( 'ymd' )
                                        ? $eventEndDate->getTimestamp()
                                        : $date->setTime( 0, 0, 0 )->getTimestamp() + DAY_IN_SECONDS ) + $gmt_offset,
                                    'padding_left'       => 0,
                                    'padding_right'      => 0,
