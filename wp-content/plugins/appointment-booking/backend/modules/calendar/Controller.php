    ...
    public function executeGetDataForAppointment()
    {
        ...
        if ( $appointment->load( $this->getParameter( 'id' ) ) ) {
            ...
            foreach ( $customers as $customer ) {
                ...
                $deposit_due = 0;
                $deposit_formatted = '';
                $deposit_payment_complete = true;
                if ( Lib\Config::depositEnabled() ) {
                    $deposit_payment_complete = false;
                    $payment_details = json_decode( $customer['payment_details'] );
                    foreach ( $payment_details->items as $item ) {
                        if ( $item->ca_id == $customer['id'] ) {
                            $deposit_due += $item->deposit_due;
                            $deposit_formatted        = $item->deposit_formatted;
                            $deposit_payment_complete = $item->deposit_payment_complete;
                        }
                    }
                }
                $response['data']['customers'][] = array(
                    ...
                    'deposit_due'               => Lib\Utils\Common::formatPrice( $deposit_due ),
                    'deposit_payment_complete'  => $deposit_payment_complete,
